## Genesis Experiment Pack v1 — Build Script
...
$packDir = Join-Path $root "deliverables\genesis-experiment-pack-v1"
...
# Experiment outputs
$experiments = @("entropy_sweep", "catastrophe_resilience", "inequality_threshold")
...
# Reproduction guide and license notes (must already exist in deliverables source)
$guideSource = Join-Path $root "deliverables\00_README_REPRODUCE.md"
...
if (Test-Path $guideSource) {
    ...
    Write-Error "MISSING: deliverables\00_README_REPRODUCE.md — create it first."
    exit 1
}
...
Write-Host ""
Write-Host "=== Genesis Experiment Pack v1 ===" -ForegroundColor Green
